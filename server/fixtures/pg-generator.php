...
$result = pg_query($conn, '
  CREATE TABLE marketing.services (
    ...
    "merchant_id" character varying(400)
) WITH (oids = false);
');
...
if (isset($_POST['event_type'])) {
  ...
  $all_sql .= 'DELETE FROM marketing.services;';
...
<p>Fill the data. Order does not matter, just note the dates. Since here the precision is 1 day, please do not set visit
  and connect to the exact same date.</p>
<p>The "Clear chains" flag will clear existing saved chains. If you do not set it, the new chains db will not be
  cleared, i.e. we'll get new conversions.</p>
...
<form method="post" action="">
  <table>
    <tr style="text-align: left">
      ...
      <th><br>User Id</th>
      <th><br>Service Id</th>
      <th><br><abbr style="color: #f00" title="Needs to match affiliate id on the Affiliate platform">Affiliate
          Id</abbr></th>
      <th><br><abbr style="color: #f00" title="Needs to match campaign id on the Affiliate platform">Campaign Id</abbr>
      </th>
      <th><br>Subtrack</th>
    ...
    <?php for ($i = 0; $i < COUNT; $i++) : ?>
      <tr>
        ...
        <td><input style="width: 140px" disabled type="date" name="event_date[<?= $i ?>]" placeholder="event date"/>
        </td>
        <td><input style="width: 100px" disabled type="text" name="track_id[<?= $i ?>]" placeholder="track id"
                   class="cell-track_id"/></td>
        <td>
        ...
        <td><input style="width: 100px" disabled type="text" name="user_id[<?= $i ?>]" class="cell-user_id"
                   placeholder="user id"/></td>
        <td><input style="width: 100px" disabled type="text" name="service_id[<?= $i ?>]" class="cell-service_id"
                   placeholder="service id"/></td>
        <td><input style="width: 120px" disabled type="text" name="affiliate_id[<?= $i ?>]" class="cell-affiliate_id"
                   placeholder="affiliate id"/></td>
        <td>
        ...
        <td><input style="width: 77px" disabled type="text" name="sub_track_id[<?= $i ?>]" class="cell-sub_track_id"
                   placeholder="subtrack"/></td>
        <td><input style="width: 77px" disabled type="number" name="quantity[<?= $i ?>]" class="cell-quantity"
                   placeholder="quantity"/></td>
        <td><input style="width: 77px" disabled type="number" step="0.01" name="amount[<?= $i ?>]" class="cell-amount"
                   placeholder="amount"/></td>
        <td><input style="width: 77px" disabled type="number" step="0.01" name="allocated[<?= $i ?>]"
                   class="cell-allocated" placeholder="allocated"/></td>
  ...
  <h2>Service id x Merchant id mapping</h2>

  <p>In order to track campaigns that were deleted from the Affiliate platform or were never there to begin with, we
    have to have a map of service_id x zignaly_user_id.</p>
  <p>You can set it on <a href="http://zzz.xfuturum.com/profile" target="_blank">the profile page</a>.
    strrife+merchant@gmail.com zignalyId is 12345</p>

  <table>
    <tr style="text-align: left">
      <th>Service id</th>
      <th>Zignaly user id</th>
    </tr>
    <tr>
      <td><input style="width: 140px" type="text" name="x_service_id[0]" placeholder="service id" value="12345"/></td>
      <td><input style="width: 100px" type="text" name="x_user_id[0]" placeholder="user id" value="12345"></td>
    </tr>
    <tr>
      <td><input style="width: 140px" type="text" name="x_service_id[1]" placeholder="service id" value="23456"/></td>
      <td><input style="width: 100px" type="text" name="x_user_id[1]" placeholder="user id" value="12345"></td>
    </tr>
    <?php for ($i = 2; $i < COUNT; $i++) : ?>
      <tr>
        <td><input style="width: 140px" type="text" name="x_service_id[<?= $i ?>]" placeholder="service id"/></td>
        <td><input style="width: 100px" type="text" name="x_user_id[<?= $i ?>]" placeholder="user id"
                   class="cell-track_id"/></td>
      </tr>
    <?php endfor; ?>
  </table>
...
<script type="text/javascript">
  function disableStuff(element) {
    ...
    } else if (element.value === 'identify') {
