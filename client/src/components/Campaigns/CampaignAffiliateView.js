...
const CampaignAffiliateView = ({ campaign, activate }) => {
  ...
  const [{loading: unarchiving}, unarchive] = useAsyncFn(() => api.post(`campaign/unarchive/${campaign._id}`).then(reloadCampaignSilently), [campaign])
          ...
          {isAffiliate && (
            isArchived
              ? (
                <div>
                  <p>
                  This campaign is archived. This campaign is hidden from your active campaign list,
                  but you continue to receive rewards affiliates converted through your links.
                  </p>
                  <p>
                    <Button link compact onClick={unarchiving ? undefined : unarchive}>
