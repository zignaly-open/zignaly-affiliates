...
const Reward = ({ campaign: { serviceType, rewardType, rewardDurationMonths, rewardValue, merchant, rewardThreshold }, short = true }) => {
  if(short) {
    ...
    return (
      <RewardWrapper>
        ...
        {rewardType === REWARD_PERCENT
          ? (
            <>
              <b>{rewardValue}% of the total revenue</b>{' '}
              during the{' '}
              <b>
                {
                  rewardDurationMonths === 0
                    ? 'lifetime'
                    : `first ${rewardDurationMonths} month${rewardDurationMonths === 1 ? '' : 's'}`
                }
              </b>{' '}
              of the user
            </>
          ...
          : (
            <b><Money value={rewardValue}/> per user</b>
          )
        }.

        Payment is done by {formatSupportedMethods(merchant)} when at least <Money value={rewardThreshold || 0}/> is reached.
      </RewardWrapper>
    );
  }
}
